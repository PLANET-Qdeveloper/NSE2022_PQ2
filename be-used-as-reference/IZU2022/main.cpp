...
#include "SDFileSystem.h" //* DONE
#include "PQ_GPS.h"       //* DONE
#include "PQ_MPU9250.h"   //! NOT USE
#include "PQ_LPS22HB.h"   //* DOING
#include "PQ_INA226.h"    //! NOT USE
#include "PQ_ES920.h"     //? CHANGE RM92A
#include "PQ_EEPROM.h"    //! NOT USE
#include "PQ_ADXL375.h"   //! NOT USE
...
int main()
{
    ...
    while (not_recovery)
    {
        ...
        case FLIGHT:
            ...
            if (!burning && (apogee || (flight_timer.read_ms() > T_APOGEE)))
                phase = SEP1;
            break;
        case SEP1:
...
void command_handler(char *command)
{
    ...
    case 0xB3: //"3" FLIGHT --> SEP1
        if (!burning && phase == FLIGHT)
            phase = SEP1;
        break;
    case 0xB4: //"4" READY or FLIGHT, SEP1, SEP2 --> EMERGENCY
        if (phase >= READY && phase <= SEP2)
            phase = EMERGENCY;
        break;
    case 0xB5: //"5" Switch SEP2_NG
        if (phase >= READY && phase <= SEP1)
            SEP2_NG = true;
        break;
    case 0xB6: //"6"
        break;
