...
try:
    ...
    # 初期化
    def init():
        global burning, detect_peak, flight_pin, sep_pin, sep_NG, phase, mission_timer_reset, init_mission_time, mission_time, mission_time_int, init_flight_time, flight_time, flight_time_int, init_sep_time, sep_time, pressure, temperature, lat, lon, alt, peak_count, apogee, separated, landed, press_index, press_buf, prev_press, ground_press, led, peak_detection_timer, read_timer, downlink_timer, temperature, block_flug, irq_called_time, gps_uart
        phase = 0
    ...
    def read():
        ...
        if phase == 3: sep_time = ticks_ms() - init_sep_time
    ...
    def debug():
        print('------------------------------------------------------------------')
        print('phase: {}, time: {}, time_reset: {}, flight_pin: {}, '.format(phase_name[phase], mission_time, mission_timer_reset, flight_pin.value()))
        print('press: {:.5f}, temp: {:.2f}, lat: {:.5f}, lon: {:.5f}, NG: {}'.format(pressure, temperature, lat, lon, sep_NG))
    ...
    def main():
        global phase, sep_pin, init_flight_time, init_sep_time, sep_NG
        ...
        while True:
            ...
            if phase == 0:  # SAFETYモード
                ...
                if (flight_time > T_SEP) and (flight_time < (T_SEP + 8000)):
                     ...
                     sep_NG = True
                if (flight_time> T_BURN) and burning == True:
                    burning = False
